...
describe('createClient', () => {
  ...
  it('skips validation and returns raw data when skipValidation is true', async () => {
    vi.stubGlobal('fetch', mockFetch(200, { notId: 123 }));
    const schema = z.object({ id: z.string() });
    const client = createClient({ ...BASE_CONFIG, skipValidation: true });
    const result = await client.sendRequest({ url: '/boards/123', schema });
    expect(result).toEqual({ notId: 123 });
  });

  it('still validates when skipValidation is explicitly false', async () => {
    vi.stubGlobal('fetch', mockFetch(200, { notId: 123 }));
    const schema = z.object({ id: z.string() });
    const client = createClient({ ...BASE_CONFIG, skipValidation: false });
    await expect(client.sendRequest({ url: '/boards/123', schema })).rejects.toThrow();
  });

  it('returns raw values without schema transforms when skipValidation is true', async () => {
