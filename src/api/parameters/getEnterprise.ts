export interface GetEnterprise {
   ...
   * precedence over the all/normal/admins value of members. If any of the member_* args are set, the member array will
   * be paginated.
   */
  memberFilter?: string;
  /**
   * This parameter expects a [SCIM-style](https://developer.atlassian.com/cloud/trello/scim/) sorting value prefixed by
   * a `-` to sort descending. If no `-` is prefixed, it will be sorted ascending. Note that the members array returned
   * will be paginated if `members` is 'normal' or 'admins'. Pagination can be controlled with member_startIndex, etc,
   * but the API response will not contain the total available result count or pagination status data.
   */
  memberSort?: string;
  /**
   * Deprecated: Please use member_sort. This parameter expects a [SCIM-style sorting
   * value](https://developer.atlassian.com/cloud/trello/scim/). Note that the members array returned will be paginated
   * if `members` is `normal` or `admins`. Pagination can be controlled with `member_startIndex`, etc, and the API
