...
Ответ, не прошедший валидацию, по умолчанию не бросает исключение — о нём сообщается один раз, а тело возвращается невалидированным, чтобы дрейф Trello API не ронял вашу интеграцию. Как это поменять — см. [`onSchemaMismatch`](/ru/guide/error-handling#schema-mismatches).
